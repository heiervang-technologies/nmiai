"""
Standalone output validator - matches the REAL server expectations.
Usage: python3 validate_output.py predictions.json

The server expects FLAT COCO-style format ONLY:
[
  {"image_id": "img_00001.jpg", "bbox": [x, y, w, h], "category_id": 42, "score": 0.95},
  ...
]

Each entry MUST have: image_id (str), bbox (list of 4 numbers), category_id (int 0-355), score (float 0-1)
"""
import json
import pathlib
...
def validate(path: pathlib.Path) -> tuple[bool, list[str]]:
    ...
    for i in check_indices:
        ...
        if not isinstance(bbox, list) or len(bbox) != 4:
            ...
            if x < 0 or y < 0:
                issues.append(f"WARNING: [{i}] bbox has negative coordinates: x={x}, y={y}")

        # category_id: must be int 0-355
        cid = entry.get("category_id")
        if not isinstance(cid, int):
            issues.append(f"[{i}] category_id must be int, got {type(cid).__name__}: {cid}")
        elif not (0 <= cid <= 355):
            issues.append(f"[{i}] category_id={cid} out of range (need 0-355)")

        # score: must be float 0-1
        score = entry.get("score")
        if not isinstance(score, (int, float)):
            issues.append(f"[{i}] score must be number, got {type(score).__name__}: {score}")
        elif not (0 <= score <= 1):
    ...
    print(f"  INFO: {len(data)} image entries, {len(data)} total predictions, {unique_images} unique images")
    if scores:
        print(f"  INFO: Score range: {min(scores):.4f} - {max(scores):.4f}")
    print(f"  INFO: Categories used: {len(categories)} (range {min(categories)}-{max(categories)})")
