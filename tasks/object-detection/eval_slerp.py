"""Quick eval of SLERP-merged YOLO on stratified val set."""
import json
import sys
...
def main():
    model_path = sys.argv[1] if len(sys.argv) > 1 else str(MODEL_PATH)
    ...
    print(f"Loaded ONNX model, provider: {session.get_providers()[0]}")
    ...
    for img_path in images:
        ...
        h, w = img.shape[:2]
        blob, ratio, (dw, dh) = letterbox(img)
        ...
        for d in dets:
            ...
            preds_by_img[img_path.name].append({
                ...
                "score": d["score"],
                "bbox": [x, y, x+bw, y+bh],  # xyxy for eval
            })
        total_dets += len(dets)
        if len(preds_by_img) % 25 == 0:
            print(f"  {len(preds_by_img)}/{len(images)} images, {total_dets} detections")

    print(f"Total: {total_dets} detections across {len(preds_by_img)} images")
    ...
    for img_path in images:
        ...
        for line in label_path.read_text().splitlines():
            ...
            anns.append({"category_id": cid, "bbox": [cx-bw/2, cy-bh/2, cx+bw/2, cy+bh/2]})
            cat_ids.add(cid)
        gt_by_img[img_path.name] = anns

    print(f"Ground truth: {sum(len(v) for v in gt_by_img.values())} boxes across {len(gt_by_img)} images, {len(cat_ids)} categories")
    ...
    print(f"\n{'='*50}")
    print(f"SLERP YOLO-only results:")
    print(f"  Detection  mAP@0.5: {det_map:.4f} ({det_map*100:.1f}%)")
    print(f"  Classification mAP@0.5: {cls_map:.4f} ({cls_map*100:.1f}%)")
    print(f"  Combined (0.7*det + 0.3*cls): {combined:.4f} ({combined*100:.1f}%)")
    print(f"\nBaseline comparison:")
    print(f"  Baseline det=74.8%, cls=92.0%, combined=80.0%")
    print(f"  SLERP    det={det_map*100:.1f}%, cls={cls_map*100:.1f}%, combined={combined*100:.1f}%")
    diff = combined - 0.800
    print(f"  Delta: {'+' if diff >= 0 else ''}{diff*100:.1f}%")
