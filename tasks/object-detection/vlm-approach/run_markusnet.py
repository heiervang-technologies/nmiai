...
MARKUSNET_CKPT = "training_output/best/best.pt"
...
class MarkusNet(nn.Module):
    ...
    def load_checkpoint(self, ckpt_path, device):
        ...
        self.cls_head.head[0].weight.data = cls_state["head.0.weight"]
        self.cls_head.head[0].bias.data = cls_state["head.0.bias"]
        self.cls_head.head[3].weight.data = cls_state["head.3.weight"]
        self.cls_head.head[3].bias.data = cls_state["head.3.bias"]
    ...
    def _preprocess_image(self, pil_img, device):
        ...
        Uses letterbox padding to preserve aspect ratio (avoids distortion).

        Returns: (pixel_values tensor, (t, h_patches, w_patches) tuple)
        """
        # Letterbox pad to preserve aspect ratio (key for classification accuracy)
