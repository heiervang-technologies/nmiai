...
- Fixed-price project + invoice percentage / "fastpris" + "delbetaling" / "fakturere X%" / "preço fixo" / "prix fixe" / "Festpreis" → create_fixed_price_project_invoice. Pass: projectName, fixedPrice, invoicePercent, customerName, customerOrgNumber, projectManagerEmail. This ONE tool call handles everything: finds/creates project, sets fixed price, creates invoice. NEVER use create_project + generic_api_call + create_invoice separately — it breaks dueDate and scorer checks.
...
- "Payment was returned/reversed" / "retourné par la banque" / "zurückgebucht" / "devuelto por el banco" / "devolvido pelo banco" / "returnert av banken": The sandbox is FRESH — there is NO pre-existing payment to reverse. You MUST do the FULL chain: 1) create_invoice (with customer, amount, product from prompt), 2) register_payment with POSITIVE amount (the original payment), 3) register_payment with NEGATIVE amount (the reversal). All three steps are required — the scorer checks that the invoice, original payment, AND reversal all exist.
